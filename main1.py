...
if os.path.isfile('db.json'):
    with open('db.json', 'r', encoding='ascii') as fp:
        USERS = json.load(fp)
        # print(USERS)
else:
    with open('db.json', 'w', encoding='ascii') as fp:
        json.dump(USERS, fp)
...
class User():
    ...
    def first_start(self, root):  # перший запуск
        ...
        password1_entry = ttk.Entry(root, width=25, textvariable=password1)
        ...
        password2_entry = ttk.Entry(root, width=25, textvariable=password2)
    ...
    def change_my_password(self, root):
        ...
        button = ttk.Button(root, text="Change my password", width=25,
                            command=lambda: self.change_password(old_pass, password1, password2))
        button.pack(pady=10)
...
class Admin(User):
    ...
    def pass_settings(self, root):
        ...
        checkbutton1 = tk.Checkbutton(root, text='Lowercase letters', variable=var1, onvalue=True, offvalue=False,
                                      command=lambda: change_settings(var1, 0))
        checkbutton1.pack()

        checkbutton2 = tk.Checkbutton(root, text='Uppercase letters', variable=var2, onvalue=True, offvalue=False,
                                      command=lambda: change_settings(var2, 1))
        checkbutton2.pack()

        checkbutton3 = tk.Checkbutton(root, text='Digits', variable=var3, onvalue=True, offvalue=False,
    ...
    def ban(self, login, type):
        ...
        if (login in USERS.keys()):
            ...
            with open('db.json', 'w', encoding='ascii') as fp:
                json.dump(USERS, fp)
            if type:
                messagebox.showinfo("Good", f"'{login}' was banned")
            else:
                # print('kek')
                USERS[login][1] = 0
                with open('db.json', 'w', encoding='ascii') as fp:
...
def check_password(password):
    ...
    if USERS['settings'][1][2]:
        if re.search('[а-яА-Я]', password) == None:
            result[2] = False
...
def author_info(root):
    clear_window(root)
    root.title('Info')
    tk.Label(root, text=f'Zhurybeda Uuriy FB-91').pack()
    tk.Label(root, text=f'Variant - 7').pack()
...
def login_window(root):
    ...
    button.pack(pady=10)
    button = ttk.Button(root, text="Info", width=25, command=lambda: author_info(root))
    button.pack(pady=40)


if get_info() == open('/home/' + os.getlogin() + '/lab2.txt', "r").read():
    root = tk.Tk()
    login_window(root)
